...
drones: Optional[dict] = {} # Eine Dictionary die keinen oder einen Typ benötigt.
...
#################################################### background Treads ###################################################
    ...
    def udp_state_receiver():
        ...
        Internal method, you normally wouldn't call this yourself.
        """
        state_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM) # Initialisiert socket
        state_socket.bind(("", Tello.STATE_UDP_PORT))

        while True:
            try:
                data, address = state_socket.recvfrom(1024) # Listen on Socket

                address = address[0] # Holt sich die Adresse des Senders
                Tello.LOGGER.debug('Data received from {} at state_socket'.format(address))

                if address not in drones: # überprüft, ob es die Adresse einer bekannten Drohne ist.
                    continue

                data = data.decode('ASCII')
                drones[address]['state'] = Tello.parse_state(data) # Schreibt in das Dictionary die Daten.
    ...
    def parse_state(state: str) -> Dict[str, Union[int, float, str]]:
        ...
        Internal method, you normally wouldn't call this yourself.
        """
        state = state.strip() # Entfernt die Leerzeichen am Anfang und Ende.
